...
class MysqlParser {
    constructor() {
        this.packets = new packets_1.default(8);
    }
    error(data) {
    ...
    process(data, is_eof) {
        ...
        data.fields.forEach(el => {
            ...
            tmp.push(xC);
            //Charset
            var charset = Buffer.from(util_1.convertBase(el.charsetNr, 2));
            tmp.push(charset);
            //Max Length Field
            var max_length_field = util_1.writeLengthCodedNumber(el.length, 4);
            tmp.push(max_length_field);
            //Column Data Type
            var column_data_type = Buffer.from([el.type]);
            tmp.push(column_data_type);
            //Flags
            var flags = util_1.writeLengthCodedNumber(el.flags, 2);
            tmp.push(flags);
            //Decimals
        ...
        data.rows.forEach(el => {
            ...
            data.fields.forEach(field => {
                ...
                if (temp) {
                    ...
                    var count_temp = util_1.writeIntToBuffer(buffer.length);
            ...
            var tmpConvertBase = util_1.convertBase(packet_id_length, 3);
