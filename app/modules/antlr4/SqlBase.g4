...
standaloneRoutineBody: routineBody EOF;
...
statement:
	...
	| INSERT INTO tableNameWithAliases columnAliases? query							# insertInto
	| UPDATE tableNameWithAliases SET updateExpression (',' updateExpression)* (WHERE whereExpression)? # update
	| DELETE FROM qualifiedName (WHERE whereExpression)?							# delete
	...
	| CREATE (OR REPLACE)? TEMPORARY? FUNCTION functionName = qualifiedName '(' (
		sqlParameterDeclaration (',' sqlParameterDeclaration)*
	)? ')' RETURNS returnType = type (COMMENT string)? routineCharacteristics routineBody #
		createFunction
	| ALTER FUNCTION qualifiedName types? alterRoutineCharacteristics	# alterFunction
	...
	| GRANT (privilege (',' privilege)* | ALL PRIVILEGES) ON TABLE? qualifiedName TO grantee =
		principal (WITH GRANT OPTION)? # grant
...
query: queryWith? queryNoWith;

queryWith: WITH RECURSIVE? namedQuery (',' namedQuery)*;
...
sqlParameterDeclaration: identifier type;
...
alterRoutineCharacteristic: nullCallClause;

routineBody: returnStatement | externalBodyReference;

returnStatement: RETURN expression;

externalBodyReference: EXTERNAL (NAME externalRoutineName)?;
...
querySpecification:
	...
	)? (WHERE where = whereExpression)? (GROUP BY groupBy)? (
...
selectItem:
	expression (AS? identifier)?	# select
...
relationPrimary:
	tableNameWithAliases	# tableName
...
updateExpression: booleanExpression;

havingExpression: booleanExpression;
...
frameBound:
	...
	| expression boundType = (PRECEDING | FOLLOWING)	# boundedFrame
		; // expression should be unsignedLiteral
...
privilege: SELECT | DELETE | INSERT | identifier;

qualifiedName: identifier ('.' identifier)*;

tableNameWithAliases: identifier (AS? identifier)?;
